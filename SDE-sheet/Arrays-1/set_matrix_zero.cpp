// question in brief: a mxn matrix is given, if  a 'zero' is present in the matrix , convert the 'ones' in the corresponding row and column to 'zero'.

// approach

// we can use the 0th row and 0th column of the given matrix itself instead of using two separate arrays

// 1) first we will traverse the 0th row and 0th column of the given matrix and if we encounter any 0 then we will set the isRow0/isCol0 variable to true 
